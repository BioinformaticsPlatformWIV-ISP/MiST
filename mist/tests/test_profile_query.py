...
class TestProfileQuery(unittest.TestCase):
    """
    Contains tests for performing profile queries.
    """
    ...
    def test_query_returns_best_matching_profiles(self) -> None:
        ...
        with testingutils.get_temp_dir() as dir_temp:
            ...
            pd.DataFrame(TestProfileQuery.TEST_PROFILES).to_csv(path_profiles, sep='\t', index=False)
            ...
            # Query the profiles
            pq = ProfileQuery(path_profiles=path_profiles, loci=list(result_by_locus.keys()))
            profiles, nb_matches = pq.query(result_by_locus)
